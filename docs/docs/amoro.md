...
# Apache Amoro With Iceberg Integrations

Iceberg format including [v1](https://iceberg.apache.org/spec/#version-1-analytic-data-tables) and [v2](https://iceberg.apache.org/spec/#version-2-row-level-deletes) is support in [Apache Amoro](https://amoro.apache.org/). Users only need to register Iceberg’s catalog in Amoro to host the table for Amoro maintenance. For detailed operation steps, refer to Managing Catalogs. Amoro maintains the performance and economic availability of Iceberg tables with minimal read/write costs through means such as small file merging, eq-delete file conversion to pos-delete files, duplicate data elimination, and file cleaning, and Amoro has no intrusive impact on the functionality of Iceberg.
